...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let buffer = SensorBufferManager::<ReadingEnvelope>::new(65_536);
    let stop = Arc::new(AtomicBool::new(false));
    let governor_cfg = AdaptivePollingConfig::default();
    ...
    let storage = StorageHandle::start(StorageConfig::default(), 256);
    ...
    let consumer = thread::spawn(move || {
        ...
        loop {
            if let Some(frame) = engine.recv_frame_timeout(Duration::from_millis(250)) {
                ...
                let storage_metrics = storage.metrics();
                ...
                if let Err(err) = storage.persist_frame(frame.clone(), Some(stats)) {
